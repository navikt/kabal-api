...
fun getParts(
...
): Triple<SakenGjelder, Klager, Prosessfullmektig?> {
    val klagePart =
        Klager(
            id = UUID.randomUUID(),
    ...
    val sakenGjelderPart =
        if (sakenGjelder?.id?.verdi == klager.id.verdi || sakenGjelder == null) {
            SakenGjelder(
                id = klagePart.id,
    ...
    val prosessfullmektigPart =
        if (klager.klagersProsessfullmektig?.id?.verdi == klager.id.verdi) {
            Prosessfullmektig(
                id = klagePart.id,
                partId = klagePart.partId,
                address = null,
    ...
    return Triple(first = sakenGjelderPart, second = klagePart, third = prosessfullmektigPart)
}

...
data class OversendtKlageAnkeV4(
    ...
    @Schema(
        description =
            "Id på forrige enhet som fattet vedtak i saken. For klager er dette typisk en vedtaksenhet, men for anker er det " +
                "typisk en klageenhet.",
        required = true,
    )
    ...
    @Schema(
        description =
            "Kan settes for å overstyre fristen i Kabal. Uten oppgitt frist settes denne til 12 uker etter " +
                "sakMottattKaTidspunkt.",
        required = false,
    )
    val frist: LocalDate?,
    @Schema(
...
fun OversendtKlageAnkeV4.toMottak(forrigeBehandlingId: UUID? = null): Mottak {
    ...
    val prosessfullmektigPart =
        if (prosessfullmektig?.id?.verdi == klagePart.partId.value) {
            Prosessfullmektig(
                id = klagePart.id,
            ...
            prosessfullmektig?.toProsessfullmektig()
        }

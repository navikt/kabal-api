...
class DokumentService(
    ...
    fun fetchDokumentlisteForBehandling(
        ...
        if (behandling.sakenGjelder.erPerson()) {
            ...
            return DokumenterResponse(
                ...
                pageReference =
                    if (dokumentoversiktBruker.sideInfo.finnesNesteSide) {
    ...
    private fun mapTema(temaer: List<Tema>): List<no.nav.klage.oppgave.clients.saf.graphql.Tema> =
        temaer.map { tema ->
            no.nav.klage.oppgave.clients.saf.graphql.Tema
                .valueOf(tema.name)
        }
    ...
    fun fetchDokumentInfoIdFromJournalpost(journalpost: Journalpost): List<String> =
        journalpost.dokumenter
            ?.filter {
                harArkivVariantformat(it)
            }?.map { it.dokumentInfoId } ?: emptyList()
    ...
    fun getJournalfoertDokumentMetadata(
        journalpostId: String,
        dokumentInfoId: String,
    ...
    private fun getMixedMemorySettingsForPDFBox(bytes: Long): StreamCacheCreateFunction = MemoryUsageSetting.setupMixed(bytes).streamCache

    fun changeTitleInPDF(
        resource: Resource,
        title: String,
    ): Resource {
        try {
                ...
                measureTimeMillis {
                    ...
                    val document: PDDocument =
                        if (resource is FileSystemResource) {
                            ...
                            Loader.loadPDF(
                                RandomAccessReadBuffer(resource.contentAsByteArray),
                                getMixedMemorySettingsForPDFBox(
    ...
    fun String.toMd5Hash(): String {
        ...
        return BigInteger(1, md.digest(this.toByteArray())).toString(16).padStart(length = 32, padChar = '0')
    }
    ...
    private fun generateTitleForDocumentsToMerge(documents: List<JournalfoertDokumentReference>): String {
        ...
        val truncatedMessage =
            if (documents.size >
                numberOfDocumentNamesToShow
            ) {
                ", ... " + (documents.size - numberOfDocumentNamesToShow) + " til"
            } else {
                ""
            }
    ...
    private fun getPreferredVariantFormatAsString(
        ...
        val journalpost =
            journalposter.find { it.journalpostId == document.first }
                ?: throw RuntimeException("Document not found in SAF")
        val dokumentInfo =
            journalpost.dokumenter?.find { it.dokumentInfoId == document.second }
                ?: throw RuntimeException("Document not found in SAF")

        val hasAccessToArkivVariant =
            dokumentInfo.dokumentvarianter.any {
                it.variantformat == Variantformat.ARKIV && it.saksbehandlerHarTilgang
            }

        val hasAccessToSladdetVariant =
            dokumentInfo.dokumentvarianter.any {
    ...
    fun updateDocumentTitle(
        ...
        dokarkivClient.updateDocumentTitlesOnBehalfOf(
            journalpostId = journalpostId,
            input =
                UpdateDocumentTitlesJournalpostInput(
                    dokumenter =
                        listOf(
    ...
    fun addLogiskVedlegg(
        ...
        val logiskVedlegg =
            dokarkivClient.addLogiskVedleggOnBehalfOf(
                dokumentInfoId = dokumentInfoId,
                title = title,
            )

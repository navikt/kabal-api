...
import no.nav.klage.oppgave.api.view.FeilregistrertEvent
import no.nav.klage.oppgave.api.view.FerdigstiltEvent
import no.nav.klage.oppgave.api.view.ForlengetBehandlingstidEvent
import no.nav.klage.oppgave.api.view.FullmektigEvent
import no.nav.klage.oppgave.api.view.HistoryEvent
import no.nav.klage.oppgave.api.view.HistoryEventType
import no.nav.klage.oppgave.api.view.HistoryEventWithPrevious
import no.nav.klage.oppgave.api.view.KlagerEvent
import no.nav.klage.oppgave.api.view.MedunderskriverEvent
import no.nav.klage.oppgave.api.view.Part
import no.nav.klage.oppgave.api.view.RolEvent
import no.nav.klage.oppgave.api.view.SaksbehandlerView
import no.nav.klage.oppgave.api.view.SattPaaVentEvent
import no.nav.klage.oppgave.api.view.TildelingEvent
import no.nav.klage.oppgave.api.view.VarsletBehandlingstidEvent
...
class HistoryService(
    ...
    fun createTildelingHistory(
        ...
        val historySorted =
            if (tildelingHistorikkSet.size == 1) {
                listOf(
                    TildelingHistorikk(
        ...
        return historySorted.zipWithNext().map { (previous, current) ->
            val previousEvent =
                HistoryEvent(
                    ...
                    actor = getSaksbehandlerView(utfoerendeIdent = previous.utfoerendeIdent, utfoerendeNavn = previous.utfoerendeNavn),
                    event =
                        TildelingEvent(
                            saksbehandler = previous.saksbehandlerident.navIdentToSaksbehandlerView(),
                            fradelingReasonId = previous.fradelingReason?.id,
                            hjemmelIdList =
                                if (previous.hjemmelIdList != null) {
            ...
            HistoryEventWithPrevious(
                ...
                actor = getSaksbehandlerView(utfoerendeIdent = current.utfoerendeIdent, utfoerendeNavn = current.utfoerendeNavn),
                event =
                    TildelingEvent(
                        saksbehandler = current.saksbehandlerident.navIdentToSaksbehandlerView(),
                        fradelingReasonId = current.fradelingReason?.id,
                        hjemmelIdList =
                            if (current.hjemmelIdList != null) {
    ...
    fun createMedunderskriverHistory(
        ...
        val historySorted =
            if (medunderskriverHistorikkSet.size == 1) {
                listOf(
        ...
        return historySorted
            ...
            .map { (previous, current) ->
                val previousEvent: HistoryEvent<MedunderskriverEvent> =
                    HistoryEvent(
    ...
    fun createRolHistory(rolHistorikk: Set<RolHistorikk>): List<WithPrevious<RolEvent>> {
        val historySorted = rolHistorikk.sortedBy { it.tidspunkt }

        return historySorted
            .zipWithNext()
            .map { (previous, current) ->
                val previousEvent: HistoryEvent<RolEvent> =
                    HistoryEvent(
        ...
        return if (feilregistrering != null) {
            return listOf(
                HistoryEventWithPrevious(
                    ...
                    actor = getSaksbehandlerView(utfoerendeIdent = feilregistrering.navIdent, utfoerendeNavn = feilregistrering.navn),
                    event =
                        FeilregistrertEvent(
    ...
    fun createFullmektigHistory(fullmektigHistorikk: Set<FullmektigHistorikk>): List<WithPrevious<FullmektigEvent>> {
        val historySorted = fullmektigHistorikk.sortedBy { it.tidspunkt }

        return historySorted
            .zipWithNext()
            .map { (previous, current) ->
                val previousEvent: HistoryEvent<FullmektigEvent> =
                    HistoryEvent(
    ...
    fun createKlagerHistory(klagerHistorikk: Set<KlagerHistorikk>): List<WithPrevious<KlagerEvent>> {
    ...
    fun createSattPaaVentHistory(sattPaaVentHistorikk: Set<SattPaaVentHistorikk>): List<WithPrevious<SattPaaVentEvent>> {
        val historySorted = sattPaaVentHistorikk.sortedBy { it.tidspunkt }

        return historySorted
            .zipWithNext()
            .map { (previous, current) ->
                val previousEvent: HistoryEvent<SattPaaVentEvent> =
                    HistoryEvent(
                        type = HistoryEventType.SATT_PAA_VENT,
                        timestamp = previous.tidspunkt,
                        actor = getSaksbehandlerView(utfoerendeIdent = previous.utfoerendeIdent, utfoerendeNavn = previous.utfoerendeNavn),
                        event =
                            previous.sattPaaVent?.let {
                                SattPaaVentEvent(
                                    from = it.from,
                                    to = it.to,
                                    reason = it.reason,
    ...
    fun createVarsletBehandlingstidHistory(
        ...
        val varsletBehandlingstidFilterered =
            varsletBehandlingstidHistorikk
                .filter { it.varsletBehandlingstid?.varselType == VarsletBehandlingstid.VarselType.OPPRINNELIG }
                .toSet()
        val historySorted =
            if (varsletBehandlingstidFilterered.size == 1) {
                listOf(
                    VarsletBehandlingstidHistorikk(
                        tidspunkt = behandlingCreated,
                        utfoerendeIdent = null,
        ...
        return historySorted
            .zipWithNext()
            .map { (previous, current) ->
                val previousEvent: HistoryEvent<VarsletBehandlingstidEvent> =
                    HistoryEvent(
    ...
    fun createForlengetBehandlingstidHistory(
        ...
        val forlengetBehandlingstidHistorikk =
            varsletBehandlingstidHistorikk
                .filter { it.varsletBehandlingstid?.varselType == VarsletBehandlingstid.VarselType.FORLENGET }
                .toSet()

        val historySorted =
            if (forlengetBehandlingstidHistorikk.size == 1) {
                listOf(
                    VarsletBehandlingstidHistorikk(
                        tidspunkt = behandlingCreated,
                        utfoerendeIdent = null,
        ...
        return historySorted
            .zipWithNext()
            .map { (previous, current) ->
                val previousEvent: HistoryEvent<ForlengetBehandlingstidEvent> =
                    HistoryEvent(
    ...
    private fun getSaksbehandlerView(

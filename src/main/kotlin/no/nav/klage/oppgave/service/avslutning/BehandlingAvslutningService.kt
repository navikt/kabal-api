...
import no.nav.klage.kodeverk.ankeutfallToInfotrygdutfall
import no.nav.klage.kodeverk.klageTilbakebetalingutfallToInfotrygdutfall
...
import no.nav.klage.oppgave.domain.kafka.AnkebehandlingAvsluttetDetaljer
import no.nav.klage.oppgave.domain.kafka.BehandlingDetaljer
import no.nav.klage.oppgave.domain.kafka.BehandlingEtterTrygderettenOpphevetAvsluttetDetaljer
...
import no.nav.klage.oppgave.domain.kafka.BehandlingEventType.BEHANDLING_ETTER_TRYGDERETTEN_OPPHEVET_AVSLUTTET
import no.nav.klage.oppgave.domain.kafka.BehandlingEventType.GJENOPPTAKSBEHANDLING_AVSLUTTET
import no.nav.klage.oppgave.domain.kafka.BehandlingEventType.KLAGEBEHANDLING_AVSLUTTET
import no.nav.klage.oppgave.domain.kafka.BehandlingEventType.OMGJOERINGSKRAVBEHANDLING_AVSLUTTET
import no.nav.klage.oppgave.domain.kafka.EventType
import no.nav.klage.oppgave.domain.kafka.ExternalUtfall
import no.nav.klage.oppgave.domain.kafka.GjenopptaksbehandlingAvsluttetDetaljer
import no.nav.klage.oppgave.domain.kafka.KafkaEvent
import no.nav.klage.oppgave.domain.kafka.KlagebehandlingAvsluttetDetaljer
import no.nav.klage.oppgave.domain.kafka.OmgjoeringskravbehandlingAvsluttetDetaljer
...
import no.nav.klage.oppgave.service.GjenopptakITrygderettenbehandlingService
import no.nav.klage.oppgave.service.GjenopptaksbehandlingService
...
class BehandlingAvslutningService(
    ...
    private fun handleKlagebehandling(klagebehandling: Klagebehandling) {
        ...
        if (klagebehandling.gosysOppgaveRequired) {
            if (klagebehandling.gosysOppgaveId != null && klagebehandling.gosysOppgaveUpdate != null &&
                !klagebehandling.ignoreGosysOppgave
            ) {
    ...
    private fun handleBehandlingEtterTrygderettenOpprettet(behandlingEtterTrygderettenOpphevet: BehandlingEtterTrygderettenOpphevet) {
        if (behandlingEtterTrygderettenOpphevet.fagsystem == Fagsystem.IT01) {
            logger.debug(
                "BehandlingEtterTrygderettenOpphevet med id ${behandlingEtterTrygderettenOpphevet.id} kommer fra Infotrygd, oppdaterer der.",
            )
            ...
            logger.debug(
                "BehandlingEtterTrygderettenOpphevet med id ${behandlingEtterTrygderettenOpphevet.id} kommer fra modernisert fagsystem, lager Kafka-melding.",
            )
            createKafkaEventForModernizedFagsystem(behandlingEtterTrygderettenOpphevet)
        } else {
            throw BehandlingAvsluttetException(
                "Ugyldig tilstand på behandlingEtterTrygderettenOpphevet med id ${behandlingEtterTrygderettenOpphevet.id}. Undersøk.",
            )
        }

        if (behandlingEtterTrygderettenOpphevet.gosysOppgaveRequired) {
            if (behandlingEtterTrygderettenOpphevet.gosysOppgaveId != null &&
                behandlingEtterTrygderettenOpphevet.gosysOppgaveUpdate != null &&
                !behandlingEtterTrygderettenOpphevet.ignoreGosysOppgave
            ) {
                logger.debug(
                    "BehandlingEtterTrygderettenOpphevet med id ${behandlingEtterTrygderettenOpphevet.id} har Gosys-oppgave, oppdaterer den.",
                )
                gosysOppgaveService.updateGosysOppgaveOnCompletedBehandling(
        ...
        if (omgjoeringskravbehandling.shouldBeSentToVedtaksinstans()) {
            ...
            if (omgjoeringskravbehandling.gosysOppgaveRequired) {
                if (!omgjoeringskravbehandling.ignoreGosysOppgave) {
                    ...
                    logger.debug(
                        "Omgjøringskravbehandling med id ${omgjoeringskravbehandling.id} har Gosys-oppgave, men den skal ignoreres.",
                    )
                }
            } else {
                logger.debug(
                    "Omgjøringskravbehandling med id ${omgjoeringskravbehandling.id} er basert på Kabal-behandling, og resultatet skal formidles til førsteinstans via en Kafka-melding.",
                )
                createKafkaEventForModernizedFagsystem(omgjoeringskravbehandling)
            }
        } else if (omgjoeringskravbehandling.shouldBeCompletedInKA()) {
            logger.debug(
                "Avslutter omgjøringskravbehandling med id ${omgjoeringskravbehandling.id} med utfall som ikke skal formidles til førsteinstans.",
            )
            if (omgjoeringskravbehandling.gosysOppgaveRequired) {
    ...
    private fun handleGjenopptaksbehandling(gjenopptaksbehandling: Gjenopptaksbehandling) {
        if (gjenopptaksbehandling.shouldBeSentToVedtaksinstans()) {
            ...
            if (gjenopptaksbehandling.gosysOppgaveRequired) {
                ...
                logger.debug(
                    "Gjenopptaksbehandling med id ${gjenopptaksbehandling.id} er basert på Kabal-behandling, og resultatet skal formidles til førsteinstans via en Kafka-melding.",
                )
            ...
            if (gjenopptaksbehandling.gosysOppgaveRequired) {
                if (gjenopptaksbehandling.gosysOppgaveId != null && gjenopptaksbehandling.gosysOppgaveUpdate != null &&
                    !gjenopptaksbehandling.ignoreGosysOppgave
                ) {
                    logger.debug("Gjenopptaksbehandling med id ${gjenopptaksbehandling.id} har Gosys-oppgave, oppdaterer den.")
                    gosysOppgaveService.updateGosysOppgaveOnCompletedBehandling(
                        behandling = gjenopptaksbehandling,
            ...
            logger.debug(
                "Gjenopptaksbehandling med id ${gjenopptaksbehandling.id} er avsluttet med utfall som ikke fører til videre behandling.",
            )
            if (gjenopptaksbehandling.gosysOppgaveRequired) {
    ...
    private fun handleGjenopptakITrygderettenbehandling(gjenopptakITrygderettenbehandling: GjenopptakITrygderettenbehandling) {
        if (gjenopptakITrygderettenbehandling.shouldCreateNewGjenopptaksbehandling()) {
            logger.debug(
                "Oppretter ny Gjenopptaksbehandling basert på GjenopptakITrygderettenbehandling fra gjenopptakITrygderettenbehandling med id ${gjenopptakITrygderettenbehandling.id}",
            )
            ...
            logger.debug(
                "Oppretter ny behandling, etter TR opphevet, basert på GjenopptakITrygderettenbehandling med id ${gjenopptakITrygderettenbehandling.id}",
            )
            createNewBehandlingEtterTROpphevetFromGjenopptakITrygderettenbehandling(gjenopptakITrygderettenbehandling)
            if (gjenopptakITrygderettenbehandling.gosysOppgaveRequired) {
                logger.debug(
                    "GjenopptakITrygderettenbehandling med id ${gjenopptakITrygderettenbehandling.id} har Gosys-oppgave, oppdaterer den.",
                )
                val kommentar =
                    "Klageinstansen har opprettet ny behandling i Kabal etter at Trygderetten opphevet saken."
            ...
            logger.debug(
                "Avslutter gjenopptakITrygderettenbehandling med id ${gjenopptakITrygderettenbehandling.id} med utfall som ikke skal formidles til førsteinstans.",
            )
            if (gjenopptakITrygderettenbehandling.gosysOppgaveRequired) {
                logger.debug(
                    "Avslutter oppgave i Gosys for gjenopptakITrygderettenbehandling med id ${gjenopptakITrygderettenbehandling.id}.",
                )
                gosysOppgaveService.avsluttGosysOppgave(
                    behandling = gjenopptakITrygderettenbehandling,
                    throwExceptionIfFerdigstilt = false,
            ...
            logger.debug(
                "GjenopptakITrygderettenbehandling med id ${gjenopptakITrygderettenbehandling.id} kommer fra modernisert fagsystem, lager Kafka-melding.",
            )
            createKafkaEventForModernizedFagsystem(gjenopptakITrygderettenbehandling)
        } else {
            if (gjenopptakITrygderettenbehandling.gosysOppgaveId != null && gjenopptakITrygderettenbehandling.gosysOppgaveUpdate != null &&
                !gjenopptakITrygderettenbehandling.ignoreGosysOppgave
            ) {
                logger.debug(
                    "GjenopptakITrygderettenbehandling med id ${gjenopptakITrygderettenbehandling.id} har Gosys-oppgave, oppdaterer den.",
                )
    ...
    private fun createKafkaEventForModernizedFagsystem(behandling: Behandling) {
        val hoveddokumenter =
            dokumentUnderArbeidCommonService
                .findHoveddokumenterByBehandlingIdAndHasJournalposter(
                    behandling.id,
                ).filter {
                    it.dokumentType in
                        listOf(
    ...
    private fun setToFinishedInInfotrygd(behandling: Behandling) {
        ...
        if (sakInKlanke.typeResultat == SakFinishedInput.TypeResultat.RESULTAT.name) {
            ...
            val utfall =
                if (sakInKlanke.sakstype != null && sakInKlanke.sakstype == "KLAGE_TILBAKEBETALING") {
            ...
            is Gjenopptaksbehandling, is GjenopptakITrygderettenbehandling -> {
                BehandlingDetaljer(
                    gjenopptaksbehandlingAvsluttet =
                        GjenopptaksbehandlingAvsluttetDetaljer(
                            avsluttet = behandling.ferdigstilling!!.avsluttetAvSaksbehandler,
                            utfall = ExternalUtfall.valueOf(behandling.utfall!!.name),
                            journalpostReferanser =
                                hoveddokumenter

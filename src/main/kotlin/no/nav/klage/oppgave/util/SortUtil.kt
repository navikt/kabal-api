...
class DokumentUnderArbeidTitleComparator : Comparator<DokumentUnderArbeid> {
    override fun compare(
        o1: DokumentUnderArbeid,
        o2: DokumentUnderArbeid,
    ): Int = compareStringsIncludingNumbers(a = o1.name, b = o2.name)
}

fun compareStringsIncludingNumbers(
    a: String,
    b: String,
): Int {
    ...
    for ((i, j) in splitA zip splitB) {
        val diff =
            if (i.first().isDigit() && j.first().isDigit()) {

...
class MellomlagretDokumentValidatorService(
    ...
    fun validateAttachment(
        ...
        if (scanForVirus) {
            ...
            logger.debug(
                "Attachment does not have virus. File size: ${file.length()} bytes (${String.format(
                    "%.2f",
                    file.length() / (1024.0 * 1024.0),
                )} MB). Time taken: ${System.currentTimeMillis() - start} ms",
            )
        } else {
            logger.debug("Skipping virus scan, document is generated by us.")
        }
